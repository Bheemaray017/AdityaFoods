...
if (isset($_SESSION['alogin'])) {
    ...
    // Get seller's rid using email
    $reg = "SELECT rid FROM tbl_registration WHERE email = ?";
    $stmt = $con->prepare($reg);
    $stmt->bind_param("s", $mail);
    $stmt->execute();
    $regid = $stmt->get_result();
    $rid_row1 = $regid->fetch_assoc();
    $rid = $rid_row1['rid'];
    $stmt->close();

    // View seller brand (not paginated)
    $viewbrand = "SELECT * FROM tbl_product WHERE rid = ?";
    $stmt = $con->prepare($viewbrand);
    $stmt->bind_param("i", $rid);
    $stmt->execute();
    $d_seller_brand = $stmt->get_result();
    $stmt->close();

    // View replied complaints (paginated)
    $viewprod = "SELECT * FROM tbl_complaints 
                 ...
                 WHERE tbl_product.rid = ? AND tbl_complaints.status = 'REPLIED' 
                 LIMIT ?, ?";
    $stmt = $con->prepare($viewprod);
    $stmt->bind_param("iii", $rid, $offset, $no_of_records_per_page);
    $stmt->execute();
    $d_seller_prod = $stmt->get_result();
    $stmt->close();

    // Display seller's basic info
    $disp = "SELECT tbl_login.user_type_id, tbl_registration.email, tbl_registration.fname, tbl_registration.lname, tbl_registration.phone_no 
             ...
             WHERE tbl_registration.email = ?";
    $stmt = $con->prepare($disp);
    $stmt->bind_param("s", $mail);
    $stmt->execute();
    $disp_result = $stmt->get_result();
    $stmt->close();

    // Now you can fetch and display $d_seller_brand, $d_seller_prod, and $disp_result
